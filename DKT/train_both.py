...
parser.add_argument('--DKT',type=bool, default= False)
...
#  We want to merge eyetracking scripts and DKT-response scripts in the same script.
...
for indexes in cross_val_list:
    ...
    for train_index in train_indexes:
        ...
        train_order_int.append(pad_order)
    
    for val_index in val_indexes:
        pad_len = max_len-len(answer_dict[train_val_data[val_index]])           
        pad_order = list(order_dict[train_val_data[val_index]]) +\
                   list((-1.) * np.ones(pad_len))               
        val_order_int.append(pad_order)
        
    #Create onehot for orders while zero-hot for no response time slice
    train_order = np.eye(SRquestion_ID+1)[np.array(train_order_int, int)] #SRquestion starts from 1,while 0 is the padding value
    val_order = np.eye(SRquestion_ID+1)[np.array(val_order_int, int)]        
